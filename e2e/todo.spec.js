...
test.describe("Todo App E2E Tests", () => {
  ...
  test("should display app title and empty state", async ({ page }) => {
    await expect(
      page.getByRole("heading", { name: "Agent Engineering Todo" })
    ).toBeVisible();
  ...
  test("should update statistics correctly with multiple operations", async ({
    page,
  }) => {
    const input = page.getByPlaceholder("Add a new todo...");
  ...
  test("should display completed todos with strikethrough", async ({
    page,
  }) => {
    const input = page.getByPlaceholder("Add a new todo...");
  ...
  test("should trim whitespace from todo title", async ({ page }) => {
    ...
    // Should display the trimmed version
